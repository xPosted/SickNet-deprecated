...
public class StatSubnetViewHeader extends JPanel {
	JLabel lblNetName;
	JLabel lblData;
	JLabel lblSes;
	/**
	 * Create the panel.
	 */
	public StatSubnetViewHeader(Subnet net) {
		setMinimumSize(new Dimension(10, 90));
		setPreferredSize(new Dimension(600, 90));
		setMaximumSize(new Dimension(32767, 90));
		baseInit();
		lblNetName.setText(net.getSubnet().getHostAddress()+"/"+net.getSuffix());
		String dataIn = Controller.processSize(net.getInputData(), 2);
		String dataOut = Controller.processSize(net.getOutptData(), 2);
		lblData.setText(dataIn+"/"+dataOut);
		lblSes.setText(net.getInputSesCount()+"/"+net.getOutputSesCount());
	...
	private void baseInit() {
		...
		lblNetName.setFont(new Font("Dialog", Font.BOLD, 25));
		...
		JLabel lblNewLabel_1 = new JLabel("Subnet statistic");
